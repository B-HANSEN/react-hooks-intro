...
const CharPicker = props => {
	// as returned from custom hook:
	const [isLoading, fetchedData] = useHttp('https://swapi.co/api/people', []);

	// remove useEffect here but useHttps() instead with custom hook:
	// useEffect(() => {
	//   setIsLoading(true)
	// }, []);

	// property that gets the adjusted data as returned by custom hook
